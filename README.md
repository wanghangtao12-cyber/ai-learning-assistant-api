...
- 在WSL和GitHub Actions的Ubuntu环境中完成依赖安装与自动化测试验证，最新回归测试为34项。
...
- 回归目标：新增功能后保持现有34项测试通过。
- 持续集成：向 `main` 分支推送，或创建目标分支为 `main` 的Pull Request时，GitHub Actions会在Ubuntu和Python 3.14环境自动运行测试，当前验证结果为 `34 passed`。
